...
void LinkedList::remove(Iterator& iter) {
	...
	if (this->begin() == this->end() || iter.ptr == --begin() || iter.ptr == end()) { //nothing should be removed from an empty list | The iterator should not be the head node | The iterator should not be the end node
		return;
	}
